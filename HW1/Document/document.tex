...
\begin{document}
    ...
    \begin{homeworkProblem}
        ...
        \begin{homeworkSection}{\homeworkProblemName(e)}
            ...
            \problemAnswer{
                ...
                see that this algorithm does not always solves the MinDistRem
                problem.
            }
        \end{homeworkSection}
    \end{homeworkProblem}
    ...
    \begin{homeworkProblem}    
        ...
        \problemAnswer{
            Too hard for me \ldots I just implemented a brute force solution to
            exhaust all possible $(S_1, S_2)$ and find the one with the minimum
            distance in \textbf{\emph{hw\_1\_2.py}}. Surely this is not
    ...
    \begin{homeworkProblem}
        ...
        \begin{homeworkSection}{\homeworkProblemName(a)}
            ...
            \problemAnswer{
                We first color all grid points with 2 colors white and black so
                that the neighboring points have different color (as in a chess
                board). 
                \\\\
                Consider two points $a$ and $b$ on the grid, if they
                have the same color, then when $b$ moves to one of its
                neighboring points, they have different colors; if they have
                different colors, then when $b$ move to one of its neighboring
                points, they have the same color. Consequently, two points of
                the same color must be separated by even steps and two points of
                different colors must be separated by odd steps. Here one step
                is defined as one move towards one of the neighboring points.
                \\\\
                Due to the rule 3 the number of steps just defined equals to the
                distance between two characters in an embedded string. If
                characters on $i$ and $j$ forms a contact, their corresponding
                grid points must be neighbors so that they have different
                colors, therefore these two points must be separated by an odd
                number of steps. As a result $|i-j|$ is also odd.
            }
        \end{homeworkSection}
        ...
        \begin{homeworkSection}{\homeworkProblemName(b)}
            ...
            \problemAnswer{
                According to problem 4(a), one character in a contact must be
                on an odd position and the other must be on an even position.
                \\\\
                If $C(S) = O(S)$, we can count the number of contacts according
                to the 1's on the odd positions. For each of these 1's that is
                not at the beginning or end of the string, its corresponding
                grid points have at most 4 neighbors, but 2 of these 4 points
                must correspond to the 2 adjacent characters in the string so
                they can not form contact pairs. Consequently each 1 on the odd
                positions but not the beginning or end of the string can have at
                most 2 contacts. For the beginning 1 and the ending 1, a similar
                analysis applies and both of them have at most 3 contacts. As a
                result, the maximum number of contact is $2(C(S) + 1)$.
                \\\\
                If $C(S) = E(S)$,  we can count the number of contacts according
                to the 1's on the even positions. Similarly, each 1 that is not
                at the end of the string (it cannot be at the beginning!) can
                have at most 2 contacts. And the 1 at the end of the string (if
                any) can have at most 3 contacts. the maximum number of contact
                is $2C(S) + 1$.
                \\\\
                In summary, the number of contacts in any legal embedding of
                $S$, cannot exceed $2(C(S) + 1)$.
             }
        \end{homeworkSection}
        ...
        \begin{homeworkSection}{\homeworkProblemName(c)}
            ...
            \problemAnswer{
                One simple example is $S=10010000000100100000001001$, and the
                embedding is shown in Figure~\ref{fig:fig_2}. This embedding has
                exactly 7 contacts and 6 1's, so its ratio of contacts to 1's is
                7/6.
                
                \begin{figure}[!h]
                    \centering
                    \includegraphics[width=0.25\columnwidth]{figs/fig_2.pdf}
                    \caption{Embedding of $S=10010000000100100000001001$.}
                    \label{fig:fig_2}
                \end{figure}
            }
        \end{homeworkSection}
        ...
        \begin{homeworkSection}{\homeworkProblemName(d)}
            ...
            \problemAnswer{
                Consider only the 1's in the string. According
                to how many neighboring 1's they have in the embedding, they can
                be categorized as follows:
                \begin{enumerate}
                    \item The 1 with 4 neighboring 1's:
                        \begin{itemize}
                            \item If this 1 is at the end or beginning of the
                            string, it can form at most 3 contacts.
                            \item If this 1 is in the middle of the string, it
                            can form at most 2 contacts.
                        \end{itemize}
                    \item The 1 with 3 neighboring 1's:
                        \begin{itemize}
                            \item If this 1 is at the end or beginning of
                            the string, it can form at most 3 contacts.
                            \item If this 1 is in the middle of the string, it
                            can form at most 2 contacts.
                        \end{itemize}
                    \item The 1 with 2 neighboring 1's, it can form at most 2
                    contacts.
                    \item The 1 with 1 neighboring 1's, it can form at most 1
                    contacts.
                    \item The 1 with 0 neighboring 1's, it can form 0 contact.
                \end{enumerate}
                According to the proof of the handshake lemma, we can count the
                total number of contacts as the summation of the number of
                contacts each 1 forms divided by 2. Then we sould like to prove
                that the average number of contacts each 1 can form is at most
                $7/3$.
                \\\\
                From the above categorization, we can see that if neither
                the end nor beginning of the string is a 1 with 3 neighboring
                1's or 4 neighboring 1's, each 1 can form at most $2 < 7/3$ contacts.
                \\\\
                If there is a 1 either at the begining or the end point of
                the string have 4 or 3 neighboring 1's, we would like to have as
                few 1's that is in the middle of the string as possible in order
                to possibly make the average number of contacts each 1 can form
                larger, since each of them can form at most 2 contacts. According to the
                number of neighboring 1's that the beginning and ending 1's
                have, we can count the minimum number of middle 1's in the
                string (a very loss lower bound though) and therefore the
                corresponding ratio of contacts to 1's. Note that if this ratio
                is smaller than $7/6$, then having any more number of middle
                points will also result in a ratio smaller than $7/6$,
                because each middle point account for at most 1 to this ratio.
                The results are shown in Table~\ref{table:4d}. Some examples of
                each cases are plotted in Figure~\ref{fig:4e1} and
                Figure~\ref{fig:4e2}.
                \\\\
                From the table we can see that the maximum raio of contacts to
                1's is $7/6$.
                
                \begin{table}[!h]
                    \renewcommand{\arraystretch}{1.3}
                    \caption{Enumeration of cases where the string has the
                    minimum number of 1's in the middle.}
                    \label{table:4d}
                    \centering
                    \begin{tabular}{ccc|cc}
                        \hline
                        case & beginning of the string  & end of the string &
                        min.
                        \# 1's in the middle & ratio of contacts to 1's \\
                        \hline
                        1 & 1 with 4 neighbor 1's & 1 with 4 neighbor 1's & 6 &
                        $\frac{6\cdot 2+2\cdot 3}{2\cdot(6 + 2)} =\frac{9}{8} $
                        \\
                        2 & 1 with 4 neighbor 1's & 1 with 3 neighbor 1's & 5 &
                        $\frac{5\cdot 2+2\cdot 3}{2\cdot(5 + 2)} = \frac{8}{7}$
                        \\
                        3 & 1 with 4 neighbor 1's & 1 with 2 neighbor 1's & 4 &
                        $\frac{4\cdot 2+3 + 2}{2\cdot(4+2)} = \frac{13}{12}$
                        \\
                        4 & 1 with 4 neighbor 1's & 1 with 1 neighbor 1's & 3 &
                        $\frac{3\cdot 2+ 3 + 1}{2\cdot(3+2)} = 1$
                        \\
                        5 & 1 with 4 neighbor 1's & 1 with 0 neighbor 1's & 4 &
                        $\frac{4\cdot 2+ 3 + 0}{2\cdot(4+2)} = \frac{11}{12}$
                        \\
                        6 & 1 with 4 neighbor 1's & 0 & 4 & $\frac{4\cdot
                        2+1\cdot 3}{2\cdot(4+1)} = \frac{11}{10}$ \\
                        7 & 1 with 3 neighbor 1's & 1 with 3 neighbor 1's & 4 &
                        $\frac{4\cdot 2+2\cdot 3}{2\cdot(4+2)} = \frac{7}{6}$
                        \\
                        8 & 1 with 3 neighbor 1's & 1 with 2 neighbor 1's & 3 &
                        $\frac{3\cdot 2+3+2}{2\cdot(3+2)} = \frac{11}{10}$
                        \\
                        9 & 1 with 3 neighbor 1's & 1 with 1 neighbor 1's & 2 &
                        $\frac{2\cdot 2+3+1}{2\cdot(2+2)} = 1$
                        \\
                        10 & 1 with 3 neighbor 1's & 1 with 0 neighbor 1's & 3 &
                        $\frac{3\cdot 2+3+0}{2\cdot(3+2)} = \frac{9}{10}$ \\
                        11 & 1 with 3 neighbor 1's & 0 & 3 & $\frac{3\cdot
                        2+1\cdot 3}{2\cdot(3+1)} = \frac{9}{8}$ \\
                        \hline
                    \end{tabular}
                \end{table}
                ...
                \begin{figure}[!H]
                    \begin{minipage}[b]{0.24\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_4_4_1.pdf}}
                        \centerline{Case 1 - 1}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_4_4_2.pdf}}
                        \centerline{Case 1 - 2}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_4_3_1.pdf}}
                        \centerline{Case 2 - 1}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.24\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_4_3_2.pdf}}
                        \centerline{Case 2 - 2}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.24\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_4_3_3.pdf}}
                        \centerline{Case 2 - 3}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.24\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_4_2_1.pdf}}
                        \centerline{Case 3 - 1}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_4_2_2.pdf}}
                        \centerline{Case 3 - 2}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_4_1_1.pdf}}
                        \centerline{Case 4 - 1}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.32\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_4_0_1.pdf}}
                        \centerline{Case 5 - 1}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_4__1.pdf}}
                        \centerline{Case 6 - 1}\medskip
                    \end{minipage}
                    \caption{
                        Different cases where the string has the minimum number
                        of 1’s in the middle. Red represents beginning and
                        ending 1's and blue represents middle 1's. Symmetrical
                ...
                \begin{figure}[!H]
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_3_1.pdf}}
                        \centerline{Case 7 - 1}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.24\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_3_2.pdf}}
                        \centerline{Case 7 - 2}\medskip
                    \end{minipage}
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_3_3.pdf}}
                        \centerline{Case 7 - 3}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.24\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_3_4.pdf}}
                        \centerline{Case 7 - 4}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.24\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_3_5.pdf}}
                        \centerline{Case 7 - 5}\medskip
                    \end{minipage}
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_2_1.pdf}}
                        \centerline{Case 8 - 1}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_2_2.pdf}}
                        \centerline{Case 8 - 2}\medskip
                    \end{minipage}
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_2_3.pdf}}
                        \centerline{Case 8 - 3}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_2_4.pdf}}
                        \centerline{Case 8 - 4}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.24\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_2_5.pdf}}
                        \centerline{Case 8 - 5}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_1_1.pdf}}
                        \centerline{Case 9 - 1}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_1_2.pdf}}
                        \centerline{Case 9 - 2}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.32\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3_0_1.pdf}}
                        \centerline{Case 10 - 1}\medskip
                    \end{minipage}
                    \hfill
                    \begin{minipage}[b]{0.16\linewidth}
                        \centering
                        \centerline{\includegraphics[width=0.8\linewidth]{figs/fig_4e_3__1.pdf}}
                        \centerline{Case 11 - 1}\medskip
                    \end{minipage}
                    \caption{Different cases where the string has the minimum number
                        of 1’s in the middle (continued).}
                    \label{fig:4e2}
                \end{figure}
            }
    ...
    \begin{appendices}
        \begin{enumerate}
            \item \textbf{\emph{hw\_1\_1.py}}
            \lstinputlisting[language=Python]{../Program/hw_1_1.py}
            \item \textbf{\emph{hw\_1\_2.py}}
            \lstinputlisting[language=Python]{../Program/hw_1_2.py}
        \end{enumerate}
