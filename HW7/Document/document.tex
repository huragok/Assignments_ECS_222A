    ...
    \begin{homeworkProblem}
        ...
        \problemAnswer{
            ...
            \begin{proof}
                ...
                adjacent only 2 two nodes $u$ and $v$, consequently, $uv$ is not
                adjacent to any node in $S$. This contradicts with the fact that
            ...
            \begin{itemize}
                ...
                is also a dominating set in $G$. Suppose for contradiction that $S$
                is not a dominating set, then there exists node $x\in G$ that is
                not adjacent to any node in $S$.
                \begin{itemize}
                    \item If $x = uv$ corresponds to some edge $(u, v)$ in $H$,
                    then we have $u\notin S$ and $v\notin S$, therefore edge
                    $(u, v)$ is not covered by $S$. This contradicts with the
                    assumption that $S$ is a node cover in $H$.
                    \item If $x = u$ corresponds to an original node in $H$,
                    since $H$ is connected, there exists node $v$ in $H$ such
                    that $(u, v)$ is an edge in $H$. Then we have $v\notin S$
                \end{itemize}
            \end{itemize}
        }
