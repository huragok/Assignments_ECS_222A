...
\begin{document}
    ...
    \begin{homeworkProblem}
        ...
        \begin{homeworkSection}{\homeworkProblemName(a)}
            ...
            \problemAnswer{
                ...
                $v$ as $v.C_l$ and $v.C_{nl}$, respectively. Also denote
                the set of colors $i$ for which $V(v, i)=V(v)$ as
                $v.Color_{max}$. Denote indicator function $\delta(x\in X) = 1$
                if $x\in X$ and 0 otherwise, and indicator function $\delta(x,y)
                = 1$ if $x=y$ and 0 otherwise. Denote the color of a leaf node
                $l$ as $l.color$.
                Then we have
                \begin{align*}
                    V(v, i) & = \sum_{l\in v.C_l}\delta(i, l.color) + \sum_{n\in
                    v.C_{nl}}(V(n)+\delta(i\in n.Color_{max})) \\
                    V(v) & = \max_{i=1,\ldots,k} V(v, i)
                \end{align*}
                Here is an explanation for this recurrence:
                \begin{itemize}
                    \item For the 1st term in the RHS of the first equation,
                    node $v$ forms a same-color edge with any leaf nodes with color $i$.
                    \item For the 2nd term in the RHS the first equation, given
                    a non-leaf child $n$ of $v$
                    \begin{itemize}
                        \item If $n$ with color $i$ can achieve $V(n)$, then
                        apart from the $V(n)$ same-color edges that the tree
                        from $n$ contains, $n$ and $v$ can also form another
                        same-color edge, therefore the maximum number of
                        same-color edges introduced by $n$ is $V(n) + 1$.
                        \item If $n$ with color $i$ can not achieve $V(n)$:
                        \begin{itemize}
                            \item By setting the color of $n$ to some color
                            $j\in n.Color_{max}$, the maximum number of
                            same-color edges introduced by $n$ is $V(n)$;
                            \item By setting the color of $n$ to some color
                            $j\notin n.Color_{max}$, the maximum number of
                            same-color edges in the tree from $n$ is at most
                            $V(n) - 1$, and there is at most one additional
                            same-color edge $(v,n)$, consequently the maximum
                            number of same-color edges introduced by $n$ is
                            at most $V(n)$.
                       \end{itemize}
                    \end{itemize}
                    In conclusion, $V(n)+\delta(i\in n.Color_{max})$ is an
                    achievable upper bound for the maximum number of same-color
                    edges introduced by $n$ if $n.parent$ is colored $i$. Also,
                    the above analysis indicate that, if $n.parent$ is colored
                    $i$, then the optimal coloring for $n$ is $i$ if $i\in
                    n.Color_{max}$ or any $j\in n.Color_{max}$ if $i\notin
                    n.Color_{max}$.
                \end{itemize}
                Note that with this recurrence we don't need to explicitly
                define a base case. The solution to the original problem, is
                simply $V(T.root)$. To determine an optimal coloring scheme,
                each non-leaf, non-root node $v$ maintains the set
                $v.Color_{max}$ and a traceback from $T.root$ will determine
                the optimal color for $v$.
            }
        \end{homeworkSection}
                    ...
                    \begin{algorithmic}[1]
                        ...
                        \FOR {$l = m$ : $l>=1$: $l--$}
                            ...
                            \[
                                ...
                                + \sum_{n\in
                                h[m].C_{nl}}(V(n)+\delta(i\in
                                n.Color_{max})),\;i=1,\ldots,k
                            \]
                            and determine $V(h[m]) =
                            \max_{i=1,\ldots,k} V(h[m], i)$ and
                            $h[m].Color_{max} = \{j|V(h[m]) = V(h[m], j)\}$.
                        \ENDFOR
                        \STATE To determine an optimal coloring scheme, color
                        $T.root$ with any $j\in T.root.Color_{max}$, then
                        starting from $T.root$, perform a BFS or DFS during
                        which the color of $v$ is set to $v.parent.color$ if it
                        is in $v.Color_{max}$, or any $j\in v.Color_{max}$ if it
                        is not.                    
                        \RETURN the
                        maximal number of edges $V(T.root)$.
                    \end{algorithmic}
                \end{algorithm}
        ...
        \begin{homeworkSection}{\homeworkProblemName(c)}
            ...
            \problemAnswer{
                The essential computation happens in Step 3 of
                Algorithm~\ref{alg:count_edges}. The first $\delta$ function can
                be evaluated in constant times, V(n) can be accessed in constant
                times and the second $\delta$ function can also be evaluated in
                constant time (e.g. using hash). Therefore, the total amount of
                time to evaluate a single $V(h[m], i)$ is $O(h[m].degree)$, i.e.
                linear w.r.t to the number of children that $h[m]$ has. Then the
                evaluation of all $k$ $V(h[m], i)$, the $V(h[m])$ and
                $h[m].Color_{max}$ takes $O(k\cdot h[m].degree)$ time. The
                traceback in Step 5 takes $O(m)$ time.
                Consequently, the time bound for this DP is
                \[
    ...
    \begin{homeworkProblem}
        ...
        \problemAnswer{
            Given the entire table $D(i, j)$ and the 3 pointers attached to each
            $D(i, j)$ for traceback in the original string alignment problem, we
            can restate the original problem as:
            \begin{quote}
                For each pair $(i, j)$, count the distinct traceback paths that
                \begin{itemize}
                    \item Arrive at cell $(i, j)$ from either cell $(i - 1, j)$
                    ($S_1(i)$ aligned to a space) or cell $(i - 1, j - 1)$
                    ($S_1(i)$ aligned to $S_2(j)$).
                    \item Continue from cell $(i, j)$ to cell $(m, n)$ (so it
                    corresponds to an optimal alignment).
                \end{itemize}
            \end{quote}
            
            Inspired by this restatement, the original problem can be readily
            decomposed into 2 separate problems. Given the directed graph
            (traceback graph) where the vertices $V$ is all the cells in the
            table and the the edges $E$ are all the pointers from the optimal
            alignment problem solution, we would like to count, for each vertex
            $(i, j)$:
            \begin{itemize}
                \item The number of distinct paths from $(0, 0)$ to $(i, j)$
                including an edge $((i-1,j), (i,j))$ or edge $((i-1, j-1),
                (i,j))$. This number is denoted as $P(i, j)$.
                \item The number of distinct paths from $(i, j)$ to $(m, n)$,
                denoted as $Q(i, j)$.
            \end{itemize}
            
            Denote the number of optimal alignments that align character $i$
            with character $j$ as $R(i, j)$, then
            \begin{align}
                R(i, j) = P(i, j)\cdot Q(i, j).
            \end{align}
            All remains is to compute all $P(i, j)$ and $P(i, j)$.
            \begin{itemize}
                \item To compute $P(i, j)$, we can make use of $M(i, j)$, the
                number of optimal alignment between $S_1[1:i]$ and $S_2[1:j]$,
                i.e. the number of distinct paths from $(0, 0)$ to $(i,
                j)$, computed in Problem 5 HW 3 with DP. We simply have
                \begin{align}
                    P(i, j) = M(i - 1, j)\delta((i-1, j), (i, j)) + M(i - 1, j -
                    1)\delta((i-1, j-1), (i, j)), \label{eq:Pij}
                \end{align}
                where $\delta(u,v) = 1$ if edge $(u,v)$ is in the traceback
                graph. Remember we have
                \begin{align}
                    M(i, j) & = M(i - 1, j)\delta((i-1, j), (i, j)) + M(i,
                    j-1)\delta((i, j-1), (i, j)) \notag \\
                    & + M(i - 1, j -
                    1)\delta((i-1, j-1), (i, j)), \\
                    M(0, 0) & = 1
                \end{align}
                \item To compute $Q(i, j)$, we can run another DP program. The
                recurence we have is
                \begin{align}
                    Q(i, j) & = Q(i+1, j)\delta((i, j), (i+1, j)) + Q(i,
                    j+1)\delta((i, j), (i, j+1)) \notag \\ & + Q(i+1,
                    j+1)\delta((i, j), (i+1, j+1)),
                \end{align}
                and the base case is $Q(m,n) = 1$.
            \end{itemize}
            In summary, we can compute all $R(i, j)$ with
            Algorithm~\ref{alg:count_align}
            \begin{algorithm}
                \caption{Count the number of optimal alignments that align
                $S_1[i]$ to $S_2[j]$ for all $(i, j)$.}
                \label{alg:count_align}
                \begin{algorithmic}[1]
                    \STATE Run the DP algorithm from the lecture to determine
                    the edit distance $D(i, j)$ and the pointers for traceback
                    for all $(i, j)$.
                    \STATE Run the DP algorithm for Problem 5 HW 2 to determine
                    $M(i,j)$, i.e. the number of distinct alignments for
                    $S_1[1:i]$ and $S_2[1:j]$ for all $(i, j)$, and compute all
                    $P(i, j)$ according to Eq.~(\ref{eq:Pij}).
                    \STATE (Start the DP algorithm to compute all $Q(i, j)$).
                    Initialize $Q(m, n) = 1$
                    \FOR {$i=m:-1:1$}
                        \FOR {$j=n:-1:1$}
                            \STATE $Q(i, j) = 0$
                            \IF {There is a pointer from $(i, j)$ to $(i+1,
                            j)$}
                                \STATE $Q(i, j) += Q(i+1, j)$.
                            \ENDIF
                            \IF {There is a pointer from $(i, j)$ to $(i,
                            j+1)$}
                                \STATE $Q(i, j) += Q(i, j+1)$.
                            \ENDIF
                            \IF {There is a pointer from $(i, j)$ to $(i+1,
                            j+1)$}
                                \STATE $Q(i, j) += Q(i+1, j+1)$.
                            \ENDIF
                            \STATE $R(i, j) = P(i, j)Q(i,j)$
                        \ENDFOR
                    \ENDFOR
                    \RETURN all $R(i, j)$.
                \end{algorithmic}
    ...
    \begin{homeworkProblem}
        ...
        \begin{homeworkSection}{\homeworkProblemName(a)}
            ...
            \problemAnswer{
                ...
                \]
                However, there are smarter ways to bulid each $T_{s,t}$. Since
                entries in $T_{s,t}$ represent all bit-wise OR of all subset of
                columns of the $(s,t)$-th block of $A$, if computed in the right
                order, each entry in $T_{s,t}$ can be computed using a single
                bit-wise OR of 2 $q$-bit vector. With this method, the
                preprocessing of $A$ takes
                \[
                    t_A = O\left(\frac{n^2}{q^2}\cdot 2^q\cdot q\right) =
                    O\left(\frac{n^22^q}{q}\right).
                \]
        ...
        \begin{homeworkSection}{\homeworkProblemName(c)}
            ...
            \problemAnswer{
                ...
                size $q$ in $C_j(s)$, $s=1,\ldots,n/q$, we need to perform $n/q$
                table look-up and $n/q - 1$ $q$-bit vector summation. In RAM
                model, the time to compute $C_j$ is
                \[
                    t_{C_j} = O\left(\frac{n}{q}\cdot\frac{n}{q}\cdot q\right) =
                    O\left(\frac{n^2}{q}\right).
                \]
        ...
        \begin{homeworkSection}{\homeworkProblemName(d)}
            ...
            \problemAnswer{
                ...
                \begin{align*}
                    ...
                    \frac{n^3}{q}\right) \notag\\
                    & = O\left(n^{2+\epsilon} +\frac{n^3}{\epsilon\log
                    n}\right)
                    \notag
                    \\
                    & = O\left(\frac{n^3}{\log n}\right)
                \end{align*}
                for all $0<\epsilon<1$. If we use the other approach to build
                each $T_{s,t}$, then
                \begin{align*}
                    t_C & = t_A + nt_{C_j} = O\left(\frac{n^22^q}{q} +
                    \frac{n^3}{q}\right) \notag\\
                    & = O\left(\frac{n^{2+\epsilon}}{\epsilon\log n}
                    +\frac{n^3}{\epsilon\log n}\right)
                    \notag
                    \\
                    & = O\left(\frac{n^3}{\log n}\right)
                \end{align*}
